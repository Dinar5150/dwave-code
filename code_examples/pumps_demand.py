...
for p in pumps:
    c1 = [(x[p][t], 1) for t in time] # Initialize contraint with variable name and bias for it in the constraint inequality
...
for p in pumps:
    printout = 'P' + str(p)
    for t in time:
        sample_val = sample[x[p][t]]
        flow_val = flow[p]
        cost_val = costs[p][t] # Debugging mess

        printout += '\t' + str(sample_val)
        total_flow += int(sample_val * flow_val)
        total_cost += int(sample_val * cost_val)
    print(printout)
